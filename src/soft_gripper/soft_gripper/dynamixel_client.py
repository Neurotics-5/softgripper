...
# limitations under the License.# Copyright 2019 The ROBEL Authors.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
...
# The following addresses assume XL motors.
# see https://emanual.robotis.com/docs/en/dxl/x/xl430-w250/for control table
...
class DynamixelClient:
    ...
    def __init__(self,
        ...
        Args:
            ...
            vel_scale: The scaling factor for the velocities. This is
                motor-dependent. If not provided uses the default scale.
            load_scale: The scaling factor for the currents. This is
                motor-dependent. If not provided uses the default scale.
        """
        ...
        self._pos_vel_load_reader = DynamixelPosVelLoadReader(
            self,
            self.motor_ids,
            ...
            load_scale=load_scale if load_scale is not None else DEFAULT_LOAD_SCALE,
        )
    ...
    def connect(self):
        """Connects to the Dynamixel motors and reboots them to ensure a clean start."""
        ...
        if self.port_handler.openPort():
            ...
            raise OSError(
                f'Failed to open port at {self.port_name} (Check that the device is powered on and connected).')

        if self.port_handler.setBaudRate(self.baudrate):
            logging.info('Succeeded to set baudrate to %d', self.baudrate)
        else:
            raise OSError(
                f'Failed to set the baudrate to {self.baudrate} (Ensure that the device is configured correctly).')

        # ✅ REBOOT MOTORS AFTER CONNECT
        logging.info('Rebooting motors...')
        self.reboot_motors(self.motor_ids)

        # Allow time for motors to recover after reboot
        time.sleep(2.0)
    ...
    def reboot_motors(self, motor_ids: Sequence[int]):
        """Sends reboot instruction (0x08) to all specified motors."""
    ...
    def read_pos_vel_load(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        return self._pos_vel_load_reader.read()
    ...
    def write_desired_pos(self, motor_ids: Sequence[int],
        ...
        positions = positions / self._pos_vel_load_reader.pos_scale
...
class DynamixelPosVelLoadReader(DynamixelReader):
    ...
    def _initialize_data(self):
        ...
        self._load_data = np.zeros(len(self.motor_ids), dtype=np.float32)

    def _update_data(self, index: int, motor_id: int):
        """Updates the data index for the given motor ID."""
        load = self.operation.getData(motor_id, ADDR_PRESENT_LOAD,
                                     LEN_PRESENT_LOAD)
        ...
        load = unsigned_to_signed(load, size=2)
        ...
        self._load_data[index] = float(load) * self.load_scale

    def _get_data(self):
        """Returns a copy of the data."""
        return (self._pos_data.copy(), self._vel_data.copy(),
                self._load_data.copy())
